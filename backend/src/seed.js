...
const generateReviews = (productId, platforms) => {
    const reviews = [];
...
async function seed() {
    ...
    await PlatformPrice_1.default.deleteMany({});
    await Review_1.default.deleteMany({});
    await PriceHistory_1.default.deleteMany({});
    for (const prodData of SEED_PRODUCTS) {
        ...
        if (availablePlatforms.length === 0)
            availablePlatforms.push('Amazon');
        ...
        pricesToSave.forEach(p => {
            if (p.currentPrice === bestPrice)
                p.isBestPrice = true;
        });
        await PlatformPrice_1.default.insertMany(pricesToSave);
        await PriceHistory_1.default.insertMany(historyToSave);
        // Reviews
        const reviewsToSave = generateReviews(product._id, availablePlatforms);
        await Review_1.default.insertMany(reviewsToSave);
    }
